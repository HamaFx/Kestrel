...
export function assertSafeOutboundUrl(value: string | URL, options: { protocols?: readonly string[]; hosts?: readonly string[] } = {}): URL {
  ...
  if (isPrivateHostname(url.hostname)) throw new Error('outbound URL resolves to a private or reserved address');
  if (options.hosts && !options.hosts.includes(url.hostname.toLowerCase())) throw new Error('outbound URL host is not allowlisted');
  return url;
}

export function isSafeOutboundUrl(value: string | URL, options: { protocols?: readonly string[]; hosts?: readonly string[] } = {}): boolean {
  try { assertSafeOutboundUrl(value, options); return true; } catch { return false; }
}
