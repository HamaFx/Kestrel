...
const patterns = [
  { name: 'private key', regex: /-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----[\\s\\S]{40,}/i },
  { name: 'OpenAI-like key', regex: /\b(?:sk|rk)-[A-Za-z0-9_-]{32,}\b/ },
  { name: 'Google API key', regex: /\bAIza[0-9A-Za-z_-]{32,}\b/ },
  { name: 'credential-bearing database URL', regex: /\b(?:postgres|postgresql):\/\/(?!hamafx:\$\{POSTGRES_PASSWORD\}@|hamafx:\$\{POSTGRES_PASSWORD:-[^}]*\}@)[^\s/:]+:[^\s@]{12,}@/i },
];
const ignoredPathPattern = /^(?:docs\/|\.agents\/|\.hermes\/|\.kiro\/|plans\/|.*(?:^|\/)(?:test|tests|__tests__|playwright-report|test-results)(?:\/|$)|.*\.(?:md|html|lock|snap))/;
const isIgnoredPath = (path) => ignoredPathPattern.test(path) || path === 'apps/web/src/app/(app)/settings/api-keys/_components/api-key-card.tsx';
...
try {
  ...
  console.error(`Git-history scan could not run: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(2);
}
...
if (findings.length) {
  ...
  for (const finding of findings) console.error(`- ${finding.hash} ${finding.path}: ${finding.type}`);
