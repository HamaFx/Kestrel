...
requireText(compose, /POSTGRES_PASSWORD:\s*\$\{POSTGRES_PASSWORD:\?/, 'Database password must be required, not silently defaulted.');
requireText(compose, /KESTREL_LOCAL_DOCKER:\s*["']?true/, 'Compose must explicitly opt into local Docker migration behavior.');
requireText(compose, /healthcheck:/, 'Compose services must expose health checks.');
requireText(appEntrypoint, /DIRECT_URL.*POSTGRES_URL_NON_POOLING/, 'App migrations must prefer a direct database URL.');
requireText(appEntrypoint, /KESTREL_LOCAL_DOCKER/, 'App entrypoint must gate the local DATABASE_URL migration fallback.');
