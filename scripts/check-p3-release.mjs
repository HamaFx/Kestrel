...
for (const file of ['Dockerfile', 'Dockerfile.worker']) {
  ...
  for (const line of lines) {
    ...
    if (!digest || !/^[a-f0-9]{64}$/i.test(digest)) failures.push(`${file}: every external FROM image needs a verified 64-character sha256 digest`);
    if (/placeholder|7f2c5d9a1b0a0f2d6f2d6f0a2e9b4a7b0f4d3c6e8a1f2c3d4/i.test(image)) failures.push(`${file}: placeholder image digest detected`);
    if (/:[^@\s]*(?:latest|main|master)$/i.test(image)) failures.push(`${file}: mutable image tag detected`);
  }
}
